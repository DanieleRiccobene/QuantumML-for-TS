...
// Struttura per salvare l'ultima posizione nota di ciascun UE
struct Pos { double x, y; };
static std::unordered_map<uint64_t, Pos> g_lastKnown; // forward-fill

/*
std::vector<Vector> ReadUePositions(const std::string& filename)
{
    std::vector<Vector> positions;
    std::set<uint64_t> seenImsis;
    ...
    while (std::getline(infile, line))
    {
        ...
        uint64_t imsi;
        if (!(iss >> timestamp >> imsi >> x >> y)) { continue; }
        
        // Only add the first position for each IMSI (UE)
        if (seenImsis.find(imsi) == seenImsis.end())
        {
            seenImsis.insert(imsi);
            positions.push_back(Vector(x, y, 3.0)); // z=3.0 as in your scenario
        }
    }
    return positions;
}

void UpdateUePositions (NodeContainer ueNodes, std::string filename)
{
    // Leggi nuove posizioni dal file
    std::vector<Vector> positions = ReadUePositions(filename);

    // Assumiamo che le posizioni siano tante quante gli UEs
    for (uint32_t i = 0; i < ueNodes.GetN() && i < positions.size(); i++)
    {
    ...
    // richiama di nuovo questa funzione dopo 1s
    Simulator::Schedule(Seconds(1.0), &UpdateUePositions, ueNodes, filename);
}
*/


std::vector<uint64_t> GetUeImsisInOrder(const NodeContainer& ueNodes) {
    std::vector<uint64_t> imsis; 
    imsis.reserve(ueNodes.GetN());
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
        Ptr<Node> ue = ueNodes.Get(i);
        uint64_t imsi = 0;
        for (uint32_t j = 0; j < ue->GetNDevices(); ++j) {
            if (auto lte = ue->GetDevice(j)->GetObject<LteUeNetDevice>()) { imsi = lte->GetImsi(); break; }
            if (auto mm  = ue->GetDevice(j)->GetObject<MmWaveUeNetDevice>()) { imsi = mm->GetImsi(); break; }
            if (auto mc  = ue->GetDevice(j)->GetObject<McUeNetDevice>()) { imsi = mc->GetImsi(); break; }
        }
...
std::vector<Vector> ReadUePositionsAt(const std::string& filename, double nowSec,
                                      const std::vector<uint64_t>& ueImsis)
{
    std::unordered_map<uint64_t, std::tuple<double,double,double>> best;
    std::ifstream infile(filename);
    std::string line;

    while (std::getline(infile, line)) {
        std::istringstream iss(line);
        double ts, x, y; uint64_t imsi;
        if (!(iss >> ts >> imsi >> x >> y)) continue;  // skip righe non valide
        if (ts <= nowSec) {
            auto it = best.find(imsi);
            if (it == best.end() || std::get<0>(it->second) < ts) {
                best[imsi] = std::make_tuple(ts, x, y);
            }
        }
    }

    std::vector<Vector> out;
    out.reserve(ueImsis.size());
    for (auto imsi : ueImsis) {
        auto it = best.find(imsi);
        if (it != best.end()) {
            double x = std::get<1>(it->second);
            double y = std::get<2>(it->second);
            g_lastKnown[imsi] = {x, y};
            out.emplace_back(x, y, 3.0);
        } else {
            // usa l'ultima posizione nota o (0,0)
            auto it2 = g_lastKnown.find(imsi);
            if (it2 != g_lastKnown.end())
                out.emplace_back(it2->second.x, it2->second.y, 3.0);
            else
                out.emplace_back(0.0, 0.0, 3.0);
        }
    }
...
void UpdateUePositions(NodeContainer ueNodes, std::string filename,
                       std::vector<uint64_t> ueImsis)
{
    double nowSec = Simulator::Now().GetSeconds();
    auto positions = ReadUePositionsAt(filename, nowSec, ueImsis);

    for (uint32_t i = 0; i < ueNodes.GetN() && i < positions.size(); ++i) {
        auto mob = ueNodes.Get(i)->GetObject<MobilityModel>();
        if (mob)
          mob->SetPosition(positions[i]);
    }

    // Log facoltativo
    NS_LOG_UNCOND("[" << nowSec << "s] Updated UE positions.");

    Simulator::Schedule(Seconds(1.0), &UpdateUePositions, ueNodes, filename, ueImsis);
}
          ...
          if (uedev || mmuedev || mcuedev)
            {
              ...
              outFile << timestamp << " " << pos.x << " " << pos.y << std::endl;
              break; // Only print once per node
            }
        }
    }
}
...
void
PeriodicPositionPrint ()
{
  static uint32_t counter = 0;
  std::ostringstream ueFile, enbFile;
  ueFile << "ues_" << counter << ".txt";
  enbFile << "enbs_" << counter << ".txt";
  PrintGnuplottableUeListToFile(ueFile.str());
  PrintGnuplottableEnbListToFile(enbFile.str());
  counter++;
  double simTime = Simulator::GetMaximumSimulationTime().GetSeconds();
  double now = Simulator::Now().GetSeconds();
  if (now + 1.0 < simTime)
  {
    Simulator::Schedule(Seconds(1.0), &PeriodicPositionPrint);
  }
}
...
static ns3::GlobalValue g_trafficModel (
    "trafficModel",
    "Type of the traffic model at the transport layer [0,6],"
    ...
    " full-buffer uplink (4),"
    " mixed full-buffer traffic 70% downlink and 30% uplink (5),"
    " mixed full-buffer traffic 30% downlink and 70% uplink (6)",
...
main (int argc, char *argv[])
{
  ...
  // Get UE positions file from environment variable or use default
  std::string uePositionsFile = std::getenv("UE_POSITIONS_FILE") ? 
                                 std::getenv("UE_POSITIONS_FILE") : 
                                 "/home/raoulraft/Scrivania/ns-3-mmwave-oran/uesPositions20.txt";
  ...
  for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
    uePositionAlloc->Add(Vector(0.0, 0.0, 3.0));
  }
  ...
  auto ueImsis = GetUeImsisInOrder(ueNodes);
  auto initPos = ReadUePositionsAt(uePositionsFile, 0.0, ueImsis);

  for (uint32_t i = 0; i < ueNodes.GetN() && i < initPos.size(); ++i)
  {
      auto mob = ueNodes.Get(i)->GetObject<MobilityModel>();
      if (mob)
      {
          mob->SetPosition(initPos[i]);
      }
  }

  Simulator::Schedule(Seconds(1.0), &UpdateUePositions, ueNodes, uePositionsFile, ueImsis);
  ...
  std::cout << "Initial UE positions:" << std::endl;
  for (uint32_t i = 0; i < initPos.size(); ++i) {
      std::cout << "UE[" << i << "] = "
                << initPos[i].x << ", "
                << initPos[i].y << ", "
                << initPos[i].z << std::endl;
  }

  for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
      auto mob = ueNodes.Get(i)->GetObject<ns3::MobilityModel>();
      std::cout << "UE[" << i << "] mobility = " << (mob ? "OK" : "NULL") << std::endl;
  }

  for (uint32_t i = 0; i < mcUeDevs.GetN(); ++i)
  {
      Ptr<NetDevice> dev = mcUeDevs.Get(i);

      auto mcUe = DynamicCast<McUeNetDevice>(dev);
      std::cout << "UE[" << i << "] McUeNetDevice = "
                << (mcUe->GetMmWavePhy() ? "OK" : "NULL") << std::endl;
  }

  for (uint32_t i = 0; i < mmWaveEnbDevs.GetN(); ++i)
  {
      Ptr<NetDevice> dev = mmWaveEnbDevs.Get(i);

      std::cout << "eNB[" << i << "] type = "
                << dev->GetInstanceTypeId().GetName()
                << std::endl;

      auto enb = DynamicCast<MmWaveEnbNetDevice>(dev);
      std::cout << "eNB[" << i << "] MmWaveEnbNetDevice = "
                << (enb ? "OK" : "NULL") << std::endl;
  ...
  // ===========================
  std::cout << "ueNodes.GetN() = " << ueNodes.GetN() << std::endl;
  std::cout << "initPos.size() = " << initPos.size() << std::endl;
  ...
  ueIps.reserve(ueNodes.GetN());
  // Assign IP address to UEs, and install applications
  for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
    {
      Ptr<Node> ueNode = ueNodes.Get (u);
      ueIps.push_back(ueIpIface.GetAddress(u));
  ...
  auto installFullBufferDl = [&] (uint32_t ueIndex) {
    PacketSinkHelper dlPacketSinkHelper ("ns3::UdpSocketFactory",
                                         InetSocketAddress (Ipv4Address::GetAny (), 1234));
    sinkApp.Add (dlPacketSinkHelper.Install (ueNodes.Get (ueIndex)));
    UdpClientHelper dlClient (ueIpIface.GetAddress (ueIndex), 1234);
  ...
  auto installFullBufferUl = [&] (uint32_t ueIndex) {
    UdpClientHelper ulClient (remoteHostAddr, portUdp);
    ulClient.SetAttribute ("Interval", TimeValue (MicroSeconds (500)));
    ulClient.SetAttribute ("MaxPackets", UintegerValue (UINT32_MAX));
    ulClient.SetAttribute ("PacketSize", UintegerValue (1280));
    clientApp.Add (ulClient.Install (ueNodes.Get (ueIndex)));
  };
  ...
  switch (trafficModel)
    {
      ...
      case 4: {
        for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
          {
            installFullBufferUl (u);
          }
      }
      break;

      case 5: {
        uint32_t dlUes = (7 * ueNodes.GetN () + 9) / 10;
        for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
          {
            if (u < dlUes)
              {
      ...
      case 6: {
        uint32_t dlUes = (3 * ueNodes.GetN () + 9) / 10;
        for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
          {
            if (u < dlUes)
              {
    ...
    default:
      NS_FATAL_ERROR (
          "Traffic model not recognized, the only possible values are [0,1,2,3,4,5,6]. Value passed: "
          << trafficModel);
  ...
  //PrintGnuplottableEnbListToFile ("enbs.txt");
   ...
   //Simulator::Schedule(Seconds(0.0), &PeriodicPositionPrint); // callback che stampa ogni secondo la posizione di tutti i nodi
